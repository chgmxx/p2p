...
@Service
public class ZdwRegisterService {
	private static final Logger logger = LoggerFactory.getLogger(ZdwRegisterService.class);
	/**
	 * 生产地址，文件目录前缀.
	 */
	private static final String OUT_PATH = "/data/upload/registration/";
	...
	public String addPledgerInfo(JSONObject json) {
		try {
	        ...
	        if (text.contains("预&nbsp;&nbsp;览")||text.contains("保&nbsp;&nbsp;存")){
	        	System.out.println("登记出质人信息成功------手动--");
//	        	JedisClient.set(key, cookieStore, 6 * 1000);
	...
	public Map<String,String> addPledgedPropertyInfo(JSONObject json) {
		Map<String,String> map = new HashMap<>();
		String filePath = OUT_PATH + "cicmorgan456" + File.separator + DateUtils.getFileDate();
		try {
			String key =json.getString("userName")+"|"+json.getString("password");
			CookieStore cookieStore = HttpUtils.getCookies(key);
	        logger.info(cookieStore.toString());
...
//	            String path ="D:\\files" + "/" +key.replace("|", "#")+"/"+ registration_number + "#"+ times + "_flow.pdf";
	         // 生产环境文件地址.
	            String path = filePath + "/" + registration_number + "_flow.pdf";
	            ...
	            System.out.println(path.split("data")[1]);
	            map.put("registration_number", registration_number);
	            map.put("modification_code", modification_code);
	            map.put("path", path.split("data")[1]);
	        	map.put("msg", "登记质押财产信息成功");
	        	return map;
	        }else {
	        	map.put("msg", "登记质押财产信息失败");
	        	return map;
	        }
			
		}catch(Exception e) {
			e.printStackTrace();
			map.put("msg", "登记质押财产信息异常");
        	return map;
		}
	}
...
//		三。质押财产信息
		 ZdwRegisterService z =new ZdwRegisterService();
		 JSONObject obj = new JSONObject();
		 obj.put("userName", "cicmorgan456");
		 obj.put("password", "cicmorgan456");
