...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/wloanproject/wloanTermProjectPlan/projectPlanList">供应链还款【P-ALL】</a></li>
		<li class="active"><a href="${ctx}/wloanproject/wloanTermProjectPlan/findByProId?proid=${proid}">供应链还款【P-A】</a></li>
	</ul>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${plans}" var="wloanTermProjectPlan">
				<tr>
					<td><a href="${ctx}/wloanproject/wloanTermProjectPlan/findUserPlanListByProPlanId?projectPlanId=${wloanTermProjectPlan.id}">${wloanTermProjectPlan.wloanTermProject.name}</a></td>
							...
							<shiro:hasPermission name="wloanproject:wloanTermProjectPlan:edit">
								<td>
									<c:if test="${wloanTermProjectPlan.state == '1' }">
										<a href="${ctx}/wloanproject/wloanTermProjectPlan/findUserPlanListByProPlanId?projectPlanId=${wloanTermProjectPlan.id}">还款</a>
									</c:if> 
									<c:if test="${wloanTermProjectPlan.state==2 }">
										<b>还款成功</b>
									</c:if>
									<c:if test="${wloanTermProjectPlan.state==3 }">
										<a href="${ctx}/wloanproject/wloanTermProjectPlan/findUserPlanListByProPlanId?projectPlanId=${wloanTermProjectPlan.id}">还款失败</a>
									</c:if>
								</td>
							</shiro:hasPermission>
						</c:when>
