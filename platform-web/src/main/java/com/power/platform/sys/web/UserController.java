...
import com.power.platform.common.beanvalidator.BeanValidators;
...
public class UserController extends BaseController {
	...
	public String list(User user, HttpServletRequest request, HttpServletResponse response, Model model) throws TimeoutException, CacheException, Exception {
	...
	public String form(User user, Model model) {
		...
		if (user.getOffice()==null || user.getOffice().getId()==null){
			user.setOffice(SessionUtils.getUser().getOffice());
		}
	...
	public String save(User user, HttpServletRequest request, Model model, RedirectAttributes redirectAttributes) {
		...
		// 清除当前用户缓存
		if (user.getLoginName().equals(SessionUtils.getUser().getLoginName())){
			...
			for (User user : list){
				try{
					if ("true".equals(checkLoginName("", user.getLoginName()))){
						user.setPassword(UserUtils.entryptPassword("123456"));
						BeanValidators.validateWithException(validator, user);
						user.setCreateBy(SessionUtils.getUser());
						user.setUpdateBy(SessionUtils.getUser());
						systemService.saveUser(user);
						successNum++;
					}else{
    ...
    public String importFileTemplate(HttpServletResponse response, RedirectAttributes redirectAttributes) {
		try {
    		...
    		List<User> list = Lists.newArrayList(); list.add(SessionUtils.getUser());
