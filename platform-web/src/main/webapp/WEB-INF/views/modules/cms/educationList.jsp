...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/notice/?type=7">教育栏目列表</a></li>
		<shiro:hasPermission name="cms:notice:edit">
			<li><a href="${ctx}/cms/notice/form?type=7">教育栏目添加</a>
			</li>
		</shiro:hasPermission> 
	</ul>
	<form:form id="searchForm" modelAttribute="notice" action="${ctx}/cms/notice/?type=7" method="post" class="breadcrumb form-search">
		...
		<%-- <label>状态：</label>
		<form:radiobuttons onclick="$('#searchForm').submit();" path="delFlag" items="${fns:getDictList('cms_notice_flag')}" itemLabel="label" itemValue="value" htmlEscape="false" /> --%>
	</form:form> 
	<sys:message content="${message}"/>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>标题</th><th>显示顺序</th><th>创建时间</th><th>是否在线</th><shiro:hasPermission name="cms:notice:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
				 	...
				 	<td>
				 		<c:if test="${notice.delFlag == 0}">
				 			<a href="${ctx}/cms/notice/edit?id=${notice.id}" 
								onclick="return confirmx('确认要${notice.state ne 0?'下线':'上线'}该新闻吗？', this.href)">
							...
							<c:if test="${notice.delFlag ne '2'}"><a href="${ctx}/cms/notice/delete?id=${notice.id}${notice.delFlag ne 0?'&isRe=true':''}" 
								onclick="return confirmx('确认要删除该新闻吗？', this.href)">删除</a></c:if>
								</c:if>
				 		</c:if>
