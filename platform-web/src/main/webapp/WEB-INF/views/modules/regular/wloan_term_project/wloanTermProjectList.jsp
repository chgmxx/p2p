...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#btnProjectInfoExport").click(function(){
			top.$.jBox.confirm("确认要导出定期项目数据吗？","系统提示",function(v,h,f){
				if(v=="ok"){
		...
		$("#btnRepayPlanInfoExport").click(function(){
			top.$.jBox.confirm("确认要导出定期项目还款计划数据吗？","系统提示",function(v,h,f){
				if(v=="ok"){
	...
	function page(n, s) {
		...
		$("#pageSize").val(s);
		// 分页时，重新定义action.
		$("#searchForm").attr("action","${ctx}/wloanproject/wloanTermProject/");
		$("#searchForm").submit();
		return false;
	} // --.
</script>
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/wloanproject/wloanTermProject/">安心投列表</a></li>
		<shiro:hasPermission name="wloanproject:wloanTermProject:edit">
			<li><a href="${ctx}/wloanproject/wloanTermProject/form">安心投创建</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="wloanTermProject" action="${ctx}/wloanproject/wloanTermProject/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
		<ul class="ul-form">
			<li>
				<label <label class="label">项目期限：</label>
			...
			<li>
				<label <label class="label">状态：</label>
			...
			<li><label <label class="label">项目编号：</label> <form:input path="sn" htmlEscape="false" maxlength="32" class="input-medium" /></li>
			<li><label <label class="label">项目名称：</label> <form:input path="name" htmlEscape="false" maxlength="55" class="input-medium" /></li>
			...
			<li>
				...
				<label class="btn">安心投<input id="project_product_type_1" type="radio" name="project_product_type" value="1" checked="checked" /></label>
			</li>
			<li class="clearfix"></li>
		</ul>
	</form:form>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="wloanTermProject">
				<tr>
					<td>
						<a href="${ctx}/wloanproject/wloanTermProject/check?id=${wloanTermProject.id}">${wloanTermProject.sn}</a>
					</td>
					<td>${wloanTermProject.id}</td>
					...
					<shiro:hasPermission name="wloanproject:wloanTermProject:edit">
						<td>
							...
							<c:if test="${wloanTermProject.state == '0' }">
								<a href="${ctx}/wloanproject/wloanTermProject/form?id=${wloanTermProject.id}" onclick="return confirmx('确认要执行【修改】操作吗？', this.href)" style="padding-right:6px;"><label class="label">修改</label></a>
							...
							<c:if test="${wloanTermProject.state == '1' }">
								<a href="${ctx}/wloanproject/wloanTermProject/form?id=${wloanTermProject.id}" onclick="return confirmx('确认要执行【修改】操作吗？', this.href)" style="padding-right:6px;"><label class="label">修改</label></a>
								<a href="${ctx}/wloanproject/wloanTermProject/check?id=${wloanTermProject.id}" onclick="return confirmx('确认要执行【提交审核】操作吗？', this.href)" style="padding-right:6px;"><label class="label">提交审核</label></a>
								<a href="${ctx}/wloanproject/wloanTermProject/delete?id=${wloanTermProject.id}" onclick="return confirmx('确认要执行【删除】操作吗？', this.href)"><label class="label">删除</label></a>
							</c:if>
							<!-- 审核（审核 + 用户类型为风控经理.）. -->
							<c:if test="${wloanTermProject.state == '2' && usertype == '9' }">
								<a href="${ctx}/wloanproject/wloanTermProject/check?id=${wloanTermProject.id}" onclick="return confirmx('确认要执行【审核】操作吗？', this.href)"><label class="label">审核</label></a>
							</c:if>
							<!-- 发布. -->
							<c:if test="${wloanTermProject.state == '3'  }">
								<a href="${ctx}/wloanproject/wloanTermProject/check?id=${wloanTermProject.id}" style="padding-right:6px;" onclick="return confirmx('确认要执行【上线】操作吗？', this.href)"><label class="label">上线</label></a>
								<%-- <a href="${ctx}/wloanproject/wloanTermProjectPlan/findAxtByProId?proid=${wloanTermProject.id}"><b>还款计划</b></a> --%>
							</c:if>
							<!-- 投标中. -->
							<c:if test="${wloanTermProject.state == '4'  }">
								<%-- <a href="${ctx}/wloanproject/wloanTermProject/check?id=${wloanTermProject.id}" style="padding-right:6px;" onclick="return confirmx('确认要执行【切标】操作吗？', this.href)"><label class="label">切标</label></a> --%>
								...
								<a href="${ctx}/wloanproject/wloanTermProjectPlan/findAxtByProId?proid=${wloanTermProject.id}" style="padding-right:6px;"><b>还款计划</b></a>
