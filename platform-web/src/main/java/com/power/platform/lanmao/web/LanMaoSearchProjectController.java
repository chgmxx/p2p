...
import com.power.platform.lanmao.search.pojo.LanMaoProject;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/lanMao/search/searchProject")
public class LanMaoSearchProjectController extends BaseController {
	@Autowired
	private LanMaoSearchProjectDataService searchProjectDataService;
	...
	@RequiresPermissions("lanMao:search:searchProject:view")
	...
	public String list(LanMaoWhiteList lanMaoWhiteList, HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		LanMaoProject project = new LanMaoProject();
		if(lanMaoWhiteList.getProjectNo()!=null && lanMaoWhiteList.getProjectNo()!="") {
		result = searchProjectDataService.searchProject(lanMaoWhiteList.getProjectNo());
		if(result.get("code").equals("调用成功")) {
			model.addAttribute("result", result);	
				...
				model.addAttribute("message", "请输入标的号");
				model.addAttribute("result", project);
			}
		return "modules/lanMao/search/searchProject";
	}
