...
import com.power.platform.common.service.ServiceException;
...
import com.power.platform.sys.dao.MenuDao;
import com.power.platform.sys.dao.OfficeDao;
import com.power.platform.sys.dao.RoleDao;
import com.power.platform.sys.dao.UserDao;
...
public class SystemService extends BaseService/* implements InitializingBean */{
	...
	public User getUserByLoginName(String loginName) {
		User user = (User)CacheUtils.get(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LOGIN_NAME_ + loginName);
		if (user == null){
	...
	public Page<User> findUser(Page<User> page, User user)  {
		...
		// 设置分页参数
		user.setPage(new Page<User>(page.getPageNo(),page.getPageSize()));
		// 执行分页查询
	...
	public List<User> findUser(User user){
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		user.getSqlMap().put("dsf", dataScopeFilter((User)user.getCurrentUser(), "o", "a"));
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveRole(Role role) {
		...
		CacheUtils.remove(UserUtils.CACHE_ROLE_ALL_LIST);
		CacheUtils.removeByCacheName(UserUtils.USER_CACHE);
		CacheUtils.remove(UserUtils.CACHE_MENU_ALL_LIST);
	}
	...
	public void saveMenu(Menu menu) {
		...
		// 清除用户菜单缓存
		CacheUtils.remove(UserUtils.CACHE_ROLE_ALL_LIST);
		CacheUtils.remove(UserUtils.USER_CACHE, UserUtils.CACHE_MENU_LIST+UserUtils.USER_CACHE_ID_+menu.getCurrentUser().getId());
		CacheUtils.remove(UserUtils.CACHE_MENU_ALL_LIST);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	/*private void saveActivitiGroup(Role role) {
		...
		// 创建用户与用户组关系
		List<IUser> userList = findUser((IUser)new User(new Role(role.getId())));
		for (IUser e : userList){
	...
	public void deleteActivitiGroup(IRole role) {
		if (!Global.isSynActivitiIndetity()){
			return;
		}
		if(role!=null) {
	...
	private void saveActivitiUser(User user) {
		...
		// 创建用户与用户组关系
		for (IRole role : user.getRoleList()) {
	 		String groupId = role.getEnname();
	 		// 如果该用户组不存在，则创建一个
		 	Group group = identityService.createGroupQuery().groupId(groupId).singleResult();
            if(group == null) {
