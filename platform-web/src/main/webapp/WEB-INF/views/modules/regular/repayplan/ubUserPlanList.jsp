...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/wloanproject/wloanTermProjectPlan/findByProId?proid=${proid}">供应链还款【P-A】</a></li>
		<li class="active"><a href="${ctx}/wloanproject/wloanTermProjectPlan/findUserPlanListByProPlanId?projectPlanId=${wloanTermProjectPlan.id}&proid=${proid}">供应链还款【C-ALL】</a></li>
	</ul>
			...
			<c:when test="${viewType != 1}">
				<shiro:hasPermission name="wloanproject:wloanTermProjectPlan:edit">
					...
					<c:if test="${wloanTermProject.isReplaceRepay == '1'}">
						<input id="btnSubmit" class="btn btn-primary" type="button" value="自动授权预处理，间接代偿确认" onclick="returnWloanTermProjectPlan('${wloanTermProjectPlan.id}','${repaymentDate}','${proid}','${wloanTermProject.isReplaceRepay}')" />&nbsp;&nbsp;&nbsp;
					</c:if>
				</shiro:hasPermission>
			</c:when>
			<c:otherwise>
    	...
    	function returnWloanTermProjectPlan(projectPlanId, repaymentDate, proid, isReplaceRepay){		
    		...
    		if(now >= repaydate){
        		...
        		if(isReplaceRepay == 0){ // 安心投还款.
	        		confirmx("确认要执行【安心投还款】操作吗？", "${ctx}/cgb/p2p/trade/bid/repay?projectPlanId="+projectPlanId+"&repaymentDate="+repaymentDate+"&ip="+ips+"&proid="+proid+"&type=2");
        		} else if (isReplaceRepay == 1) { // 供应链还款.
        			confirmx("确认要执行【间接代偿】操作吗？", "${ctx}/lm/p2p/compensatory/indirectCompensatory?projectPlanId="+projectPlanId+"&repaymentDate="+repaymentDate+"&ip="+ips+"&proid="+proid+"&type=2");
        		}
    		} else {
    			alertx("未到还款日，不能执行【间接代偿】操作！");
    			return;
