...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 导出.
			$("#btnExportLendersTransDetail").click(function(){
			...
			//导出存管宝账户对账
			$("#btnExportCheckAccount").click(function(){
				top.$.jBox.confirm("确认要导出[存管宝账户对账]结果操作吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#transDetailRadioType").val($('input[name="trans_Detail_type"]:checked').val());
						$("#searchForm").attr("action","${ctx}/cgb/cgbUserTransDetail/checkaccount");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
				top.$('.jbox-body .jbox-icon').css('top','55px');
...
<body>
	...
	<form:form id="searchForm" modelAttribute="cgbUserTransDetail" action="" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns">
				...
				<input id="btnExportLendersTransDetail" class="btn btn-primary" type="button" value="导出" />
				<input id="btnExportCheckAccount" class="btn btn-primary" type="button" value="账户对账" />
			</li>
			<li>
				<form:hidden path="transDetailRadioType"/>
				<label class="btn">出借人~<input id="lenders_id" type="radio" name="trans_Detail_type" value="1" checked="checked" onclick="return page();" /></label>
				<label class="btn">借款人~<input id="borrowers_id" type="radio" name="trans_Detail_type" value="2" onclick="return page();" /></label>
			</li>
			<li class="clearfix"></li>
		</ul>
	</form:form>
