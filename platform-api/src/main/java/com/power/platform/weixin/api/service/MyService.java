package com.power.platform.weixin.api.service;
...
import com.power.platform.weixin.api.process.HttpMethod;
...
import com.power.platform.weixin.api.process.WxApi;
import com.power.platform.weixin.api.process.WxApiClient;
import com.power.platform.weixin.api.process.WxMessageBuilder;
import com.power.platform.weixin.api.vo.Matchrule;
...
public class MyService {
	...
	public String processMsg(MsgRequest msgRequest,MpAccount mpAccount){
		...
		if(StringUtils.isEmpty(respXml)){
			MsgText text = msgTextDao.getMsgTextByInputCode(MsgType.SUBSCRIBE.toString());
			if(text != null){
	...
	private String processEventMsg(MsgRequest msgRequest){
		...
		if(MsgType.SUBSCRIBE.toString().equals(msgRequest.getEvent())){//订阅消息
			MsgText text = msgTextDao.getMsgTextByInputCode("subscribe");
			if(text != null){
			...
			MsgText text = msgTextDao.getMsgTextByInputCode(MsgType.UNSUBSCRIBE.toString());
			if(text != null){
				return MsgXmlUtil.textToXml(WxMessageBuilder.getMsgResponseText(msgRequest, text));
			}
		}else{//点击事件消息
				...
				if(key.startsWith("_fix_")){
					...
					if(!StringUtils.isEmpty(baseIds)){
						...
						if(idArr.length > 1){//多条图文消息
							List<MsgNews> msgNews = msgNewsDao.listMsgNewsByBaseId(idArr);
							if(msgNews != null && msgNews.size() > 0){
								return MsgXmlUtil.newsToXml(WxMessageBuilder.getMsgResponseNews(msgRequest, msgNews));
							}
						}else{//图文消息，或者文本消息
							MsgBase msg = msgBaseDao.get(baseIds);
							if(msg.getMsgType().equals(MsgType.Text.toString())){
								MsgText text = msgTextDao.getMsgTextByBaseId(baseIds);
								if(text != null){
									return MsgXmlUtil.textToXml(WxMessageBuilder.getMsgResponseText(msgRequest, text));
								}
							}else{
	...
	private String prepareMenus(List<AccountMenu> menus,Matchrule matchrule) {
		if(!CollectionUtils.isEmpty(menus)){
			...
			for(AccountMenu m : menus){
				if(m.getParentid() == "0" || m.getParentid().equals("0")){//一级菜单
					parentAM.add(m);
				}else{//二级菜单
	...
	private JSONObject getMenuJSONObj(AccountMenu menu){
		...
		if("click".equals(menu.getMtype())){//事件菜单
			if("fix".equals(menu.getEventType())){//fix 消息
				obj.put("key", "_fix_" + menu.getMsgBase().getId());//以 _fix_ 开头
			}else{
