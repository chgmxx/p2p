...
<body>
	...
	<form:form id="inputForm" modelAttribute="riskManagement" action="${ctx}/riskmanagement/riskManagementMessage/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">企业名称：${riskManagement.companyName}</label><label class="control-label">操作类型：签约</label>
		</div>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${annexFiles}" var="annexFile">
				<tr>
					<!-- <td>${fns:getDictLabel(annexFile.type, 'risk_management', '')}</td> -->
					<td>
					...
					<td>
					<a href="${ctx}/sys/annexfile/formSign?returnUrl=${annexFile.returnUrl}&otherId=${annexFile.otherId}&dictType=${annexFile.dictType}&id=${annexFile.id}&title=${annexFile.title}">修改</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<script type="text/javascript">
    	function addDatum(otherId,returnUrl,dictType,title,type){
			location.href="${ctx}/sys/annexfile/formSign?otherId="+otherId+"&returnUrl="+returnUrl+"&dictType=risk_management&title="+title+"&type="+type;		
    	}
    	function pic(){
    		alert("11");
