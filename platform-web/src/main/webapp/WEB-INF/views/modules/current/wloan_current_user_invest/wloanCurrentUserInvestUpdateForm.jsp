...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/wloan_current_user_invest/wloanCurrentUserInvest/updateForm?id=${wloanCurrentUserInvest.id}">活期客户投资<shiro:hasPermission name="wloan_current_user_invest:wloanCurrentUserInvest:edit">${not empty wloanCurrentUserInvest.id?'修改':'添加'}</shiro:hasPermission> <shiro:lacksPermission name="wloan_current_user_invest:wloanCurrentUserInvest:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="wloanCurrentUserInvest" action="${ctx}/wloan_current_user_invest/wloanCurrentUserInvest/updateSave" method="post" class="form-horizontal">
		<form:hidden path="id" />
		<sys:message content="${message}" />
		<div class="control-group">
			<label class="control-label">客户ID：</label>
			<div class="controls">
				<form:input path="userInfo.id" htmlEscape="false" class="input-xlarge " readonly="true" />
			</div>
		</div>
		<div class="control-group">
