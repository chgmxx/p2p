...
//					String realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + principal + "/" + ss[0] + ss[1];
					String realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + "sharedFiles" + "/" + ss[0] + ss[1];
		...
		// 快捷上传，自动创建当前文件夹，并上传到该路径
			...
			String realPath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + "sharedFiles" + "/" + type + (currentFolder != null ? currentFolder : "");
			FileUtils.createDirectory(FileUtils.path(realPath));
		}
		// System.out.println("------------------------");
