	...
	function selectLoanApplyId(id){
		$.ajax({
			...
			success : function(data) {
				$("#loanIdCard").val(data.certificateNo); // 身份证号码.
				$("#companyName").val(data.name + "[借款]"); // 姓名.
				$("#loanUser").val(data.name); // 姓名.
				$("#loanPhone").val(data.phone); // 联系电话.
				$("#cashierUser").val(data.name); // 姓名.
				$("#cashierIdCard").val(data.certificateNo); // 身份证号码.
				$("#cashierBankNo").val(data.bankAccountNo); // 银行卡号码.
				$("#cashierBankPhone").val(data.bankCardPhone); // 银行卡预留手机.
				$("#cashierBankAdderss").val(data.bankName); // 开户行名称.
				$("#remarks").val("移动端APP借款信息"); // 备注信息.
			},
			error : function(data) {
				alert("程序异常");
			}
...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/wloan_subject/wloanSubject/addForm?id=${wloanSubject.id}">融资主体添加</a></li>
		<li class="active"><a href="${ctx}/wloan_subject/wloanSubject/insertForm?id=${wloanSubject.id}">融资主体[借款]新增</a></li>
	</ul>
	<br />
	<form:form id="personalForm" modelAttribute="wloanSubject" action="${ctx}/wloan_subject/wloanSubject/addSave" method="post" class="form-horizontal">
		...
		<div id="personal_id" style="display: block;">
			...
			<div class="control-group">
				<div class="span6">
					...
					<div class="controls">
						<form:select path="loanApplyId" class="input-xlarge required" style="width:264px" id="loanApplyId">
		...
		<div class="control-group">
			<div class="span6">
				<div id="g_name" style="display: block;"><label class="control-label">个人名称：</label></div>
			...
			<div class="span6">
				<label class="control-label">借款人：</label>
				<div class="controls">
					<form:input path="loanUser" htmlEscape="false" maxlength="11" class="input-large required"/>
					<span class="help-inline"><font color="red">*</font></span>
				</div>
			</div>
			<div class="span6">
