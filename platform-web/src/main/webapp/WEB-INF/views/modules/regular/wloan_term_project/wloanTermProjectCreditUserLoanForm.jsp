...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 是否代偿还款标识-联动-代偿户.
			$('#isReplaceRepay').change(function(){
				// console.log("isEntrustedPay：" + $(this).children('option:selected').val());
				var isReplaceRepay = $(this).children('option:selected').val();
				// console.log("isEntrustedPay：" + isEntrustedPay);
				CreditUserInfosByisReplaceRepay(isReplaceRepay);
			});// --

			// 借款申请-联动-代偿户.
			$('#creditUserApplyId').change(function(){
				var creditUserApplyId = $(this).children('option:selected').val();
				CreditApplyByReplaceRepayUser(creditUserApplyId);
			});// --
		...
		// 
		function financingAmount(id){
			// $.ajax({
				// url : "${ctx}/wloanproject/wloanTermProject/selectSubjects?id=" + id, 
				// type : "post", 
				// success : function(data) {
					// $("#amount").val(data.amount); // 融资金额.
					// $("#virtualAmount").val(data.amount); // 融资金额.
					// $("#maxAmount").val(data.amount); // 最大投资金额.
					// $("#virtualMaxAmount").val(data.amount); // 最大投资金额.
					// $("#span").val(data.span); // 借款期限.
					// $("#virtualSpan").val(data.span); // 借款期限.
					// $("#purpose").val(data.borrPurpose); // 借款用途.
					// $("#virtualPurpose").val(data.borrPurpose); // 借款用途.
				// },
				// error : function(data) {
					// alert("程序异常");
				// }
		...
		function Init_Wloan_Project_Choice() {
			...
			var credit_user_apply_id = $('#hidden_credit_user_apply_id').val(); // 借款申请ID.
			// var project_data_id = $('#hidden_project_data_id').val(); // 借款资料ID.
			// console.log("subjectId：" + subjectId);
			InitWloanSubjectListByisEntrustedPay(isEntrustedPay, subjectId); // 初始化融资主体.
			InitCreditUserApplysByWloanSubjectId(subjectId, credit_user_apply_id); // 初始化借款申请.
			// InitCreditInfosByWloanSubjectId(subjectId, project_data_id); // 初始化借款资料.
			// --.
			var isReplaceRepay = $('#isReplaceRepay').children('option:selected').val(); // 代偿标识.
			var hiddenReplaceRepayId = $('#hidden_replace_repay_id').val(); // 代偿还款人ID.
			// console.log("代偿还款人ID：" + hiddenReplaceRepayId);
			InitCreditUserInfosByisReplaceRepay(isReplaceRepay, hiddenReplaceRepayId);
			// console.log("isEntrustedPay：" + isEntrustedPay);
		...
		/**
		 * FN: InitCreditUserInfosByisReplaceRepay.
		 * DESC: 初始化，是否代偿还款标识-联动-代偿户.
		 */
		function InitCreditUserInfosByisReplaceRepay(isReplaceRepay, hiddenReplaceRepayId){
			
			// console.log("value = " + isReplaceRepay);
			if(isReplaceRepay == '0'){
				document.getElementById("replace_repay_id").style.display = "none";
				$("#replaceRepayId").val("");
				document.getElementById("replace_repay_residence_id").style.display = "none";
				$("#replaceRepayResidence").val(""); // 清除代偿方住所.
				replaceRepayElectronicSignImageDelAll(); // 清除电子签章.
			} else {
				document.getElementById("replace_repay_id").style.display = "block";
				$.ajax({
					url : "${ctx}/wloanproject/wloanTermProject/isReplaceRepay?isReplaceRepay=" + isReplaceRepay, 
					type : "post", 
					success : function(data) {
						var table = data.creditUserInfos;
						var $replaceRepayId = $("#replaceRepayId");
						$replaceRepayId.empty();// 首先清空select现在有的内容.
						$replaceRepayId.append('<option value="">请选择</option>');
						// console.log("value = " + $subjectId.val());
						// console.log("value = " + $subjectId.text());
						// console.log("option:selected = " + $("#subjectId").find("option:selected").text());
						for (var i = 0; i < table.length; i++) {
							var item = table[i];
							if(item.id == hiddenReplaceRepayId){
								// console.log("item.id：" + item.id + ",hiddenReplaceRepayId：" + hiddenReplaceRepayId);
								$replaceRepayId.append("<option  selected='selected' value=" + item.id + ">" + item.name + "[" + item.creditScore + "]" + "</option>");
								$("span.select2-chosen:eq(5)").text(item.name + "[" + item.creditScore + "]");
							} else {
								$replaceRepayId.append("<option  value=" + item.id + ">" + item.name + "[" + item.creditScore + "]" + "</option>");
							}
						}
						// 上次选中的值重新赋值.
						// console.log($("option:selected").text());
				...
				document.getElementById("replace_repay_residence_id").style.display = "block";
			}
		}// --
		 ...
		 * DESC:初始化借款申请.
		 */
		function InitCreditUserApplysByWloanSubjectId(subjectId, credit_user_apply_id){
			 $.ajax({
					url : "${ctx}/wloanproject/wloanTermProject/creditUserApplysByWloanSubjectId?wloanSubjectId=" + subjectId, 
					type : "post", 
					success : function(data) {
						var table = data.ztmgLoanApplyAndBasicInfoPojo.creditUserApplys;
						if(typeof(table) == "undefined"){
							var $creditUserApplyId = $("#creditUserApplyId");
							$creditUserApplyId.empty();// 首先清空select现在有的内容.
						} else {
							var $creditUserApplyId = $("#creditUserApplyId");
							$creditUserApplyId.empty();// 首先清空select现在有的内容.
							$creditUserApplyId.append('<option value="">请选择</option>');
							// console.log("value = " + $subjectId.val());
							// console.log("value = " + $subjectId.text());
							// console.log("option:selected = " + $("#subjectId").find("option:selected").text());
							for (var i = 0; i < table.length; i++) {
								var item = table[i];
								if(item.id == credit_user_apply_id){
									$creditUserApplyId.append("<option selected='selected' value=" + item.id + ">" + item.creditApplyName + "</option>");
								} else {
									$creditUserApplyId.append("<option  value=" + item.id + ">" + item.creditApplyName + "</option>");
								}
							}
							// 上次选中的值重新赋值.
							$("span.select2-chosen:eq(3)").text($("#creditUserApplyId option:selected").text());
							// console.log($("option:selected").text());
							// console.log($("span.select2-chosen:eq(1)").text());
		...
		// 初始化借款资料.
		function InitCreditInfosByWloanSubjectId(wloanSubjectId, project_data_id){
			$.ajax({
				url : "${ctx}/wloanproject/wloanTermProject/creditInfosByWloanSubjectId?wloanSubjectId=" + wloanSubjectId, 
				type : "post", 
				success : function(data) {
					var table = data.creditInfos;
					var $projectDataId = $("#projectDataId");
					$projectDataId.empty();// 首先清空select现在有的内容.
					$projectDataId.append('<option selected="selected" value="">请选择</option>');
					// console.log("value = " + $subjectId.val());
					// console.log("value = " + $subjectId.text());
					// console.log("option:selected = " + $("#subjectId").find("option:selected").text());
					for (var i = 0; i < table.length; i++) {
						var item = table[i];
						if(item.id == project_data_id){
							$projectDataId.append("<option selected='selected' value=" + item.id + ">" + item.name + "</option>");
						} else {
							$projectDataId.append("<option value=" + item.id + ">" + item.name + "</option>");
						}
					}
					// 上次选中的值重新赋值.
		...
		function CreditUserApplysByWloanSubjectId(wloanSubjectId){
			$.ajax({
				url : "${ctx}/wloanproject/wloanTermProject/creditUserApplysByWloanSubjectId", 
				type : "post",
				async : false,
				contentType : "application/x-www-form-urlencoded",
				dataType : "json",
				data : {
					"wloanSubjectId" : wloanSubjectId
				},
				success : function(data) {
					var table = data.ztmgLoanApplyAndBasicInfoPojo.creditUserApplys;
					if(typeof(table) == "undefined"){
						...
						var $creditUserApplyId = $("#creditUserApplyId");
						$creditUserApplyId.empty();// 首先清空select现在有的内容.
						$creditUserApplyId.append('<option selected="selected" value="">请选择</option>');
						// console.log("value = " + $subjectId.val());
						// console.log("value = " + $subjectId.text());
						// console.log("option:selected = " + $("#subjectId").find("option:selected").text());
						for (var i = 0; i < table.length; i++) {
		...
		// 融资主体-联动-借款资料.
		function CreditInfosByWloanSubjectId(wloanSubjectId){
			$.ajax({
				url : "${ctx}/wloanproject/wloanTermProject/creditInfosByWloanSubjectId?wloanSubjectId=" + wloanSubjectId, 
				type : "post", 
				success : function(data) {
		...
		// 是否代偿还款标识-联动-代偿户.
		function CreditUserInfosByisReplaceRepay(isReplaceRepay){
			...
			if(isReplaceRepay == '0'){
				...
				$.ajax({
					...
					success : function(data) {
						...
						var $replaceRepayId = $("#replaceRepayId");
						$replaceRepayId.empty();// 首先清空select现在有的内容.
						$replaceRepayId.append('<option selected="selected" value="">请选择</option>');
						// console.log("value = " + $subjectId.val());
						// console.log("value = " + $subjectId.text());
						// console.log("option:selected = " + $("#subjectId").find("option:selected").text());
						for (var i = 0; i < table.length; i++) {
							var item = table[i];
							$replaceRepayId.append("<option  value=" + item.id + ">" + item.name + "</option>");
						}
						// 上次选中的值重新赋值.
						$("span.select2-chosen:eq(5)").text("请选择");
						// console.log($("option:selected").text());
						// console.log($("span.select2-chosen:eq(1)").text());
					},
					error : function(data) {
		...
		// 借款申请-联动-代偿户.
		function CreditApplyByReplaceRepayUser(creditApplyId){

			// console.log("creditApplyId = " + creditApplyId);
			$.ajax({
				url : "${ctx}/wloanproject/wloanTermProject/creditApplyByReplaceRepayUser?creditApplyId=" + creditApplyId, 
				type : "post", 
				success : function(data) {
					var creditUserApply = data.creditUserApply;
					var allOptions = $("#replaceRepayId option");
					for(var i = 0; i < allOptions.length; i++){
						if(creditUserApply.replaceUserId == allOptions[i].value){
							// console.log("value = " + allOptions[i].value);
							// console.log("creditUserApply.replaceUserId = " + creditUserApply.replaceUserId);
							allOptions[i].selected = true;
							$("span.select2-chosen:eq(5)").text(allOptions[i].text);
						}
					}
				},
				error : function(data) {
		...
		// 查找项目编号是否存在.
		function findProSnExist(sn){
			$.ajax({
				url : "${ctx}/wloanproject/wloanTermProject/findProSnExist?sn=" + sn, 
				type : "post", 
				success : function(data) {
					// console.log("是否存在：" + data.isSnExist);
					if(data.isSnExist){
						$('#sn').removeClass("valid");
						$('#sn').addClass("error");
						$('#sn').after('<b for="sn" class="error" style="display: block;">该项目编号已存在，请重新输入...</b>&nbsp;');
					} else {
						$('#sn').next("b").remove();
					}
				},
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active">
			<a href="${ctx}/wloanproject/wloanTermProject/creditUserLoanForm?id=${wloanTermProject.id}">供应链${not empty wloanTermProject.id?'修改':'创建'}</a>
		</li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="wloanTermProject" action="${ctx}/wloanproject/wloanTermProject/creditUserLoanSave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="span6">
				<label class="control-label">借款申请：</label>
				<div class="controls">
					<form:select path="creditUserApplyId" class="input-xlarge required" style="width:264px">
						<form:option value="" label="请选择"/>
					</form:select>
					<input id="hidden_credit_user_apply_id" type="hidden" value="${wloanTermProject.creditUserApplyId}" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
			<!-- 
			<div class="span6">
				<label class="control-label">借款资料：</label>
				<div class="controls">
					<form:select path="projectDataId" class="input-xlarge required" style="width:264px">
						<form:option value="" label="请选择"/>
						<c:forEach items="${creditDocs}" var="creditDocs">
							<form:option value="${creditDocs.id }" label="${creditDocs.name }"/>
						</c:forEach>
					</form:select>
					<input id="hidden_project_data_id" type="hidden" value="${wloanTermProject.projectDataId}" />
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
			 -->
		...
		<div class="control-group">
			<div class="span6">
				...
				<div class="controls">
					<form:select path="isReplaceRepay" class="input-xlarge required" style="width:264px" disabled="disabled">
						<form:option value="1" label="是"/>
						<%-- <form:option value="0" label="否"/> --%>
					</form:select>
					<span class="help-inline"><font color="red">*</font> </span>
			...
			<div id="replace_repay_id" style="display: none;">
				<div class="span6">
					<label class="control-label">代偿还款人：</label>
					<div class="controls">
						<form:select path="replaceRepayId" class="input-xlarge required" style="width:264px">
							<form:option value="" label="请选择"/>
							<c:forEach items="${creditUserInfos}" var="creditUserInfo">
								<form:option value="${creditUserInfo.id}" label="${creditUserInfo.name}【代偿户】"/>
							</c:forEach>
						</form:select>
						<input id="hidden_replace_repay_id" type="hidden" value="${wloanTermProject.replaceRepayId}" />
						<span class="help-inline"><font color="red">*</font> </span>
					</div>
				</div>
			</div>
		...
		<div class="control-group">
			...
			<div class="span6">
				...
				<div class="controls">
					<form:input path="annualRate" htmlEscape="false" class="input-xlarge number required"  onkeyup="value=value.replace(/[^\d.]/g,'')" style="width:230px" />
		...
		<div class="control-group">
			<div class="span6">
				...
				<div class="controls">
					<!-- <input type="text" readonly="readonly" value="按月付息到期还本" style="width:250px"> -->
			...
			<div class="span6">
				...
				<div class="controls">
					<form:input path="interestRateIncrease" htmlEscape="false" class="input-xlarge number required"  onkeyup="value=value.replace(/[^\d.]/g,'')" style="width:230px" />
					<span class="help-inline">%</span> <span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</div>
			...
			<div class="span6">
				...
				<div class="controls">
					<form:select path="projectProductType" class="input-xlarge required" style="width:264px">
						<form:option value="2" label="供应链类"/>
					</form:select>
					<span class="help-inline"><font color="red">*</font></span>
				</div>
			</div>
		...
		<div class="control-group">
			<div class="span6">
				...
				<div class="controls">
					<form:select path="isCanUseCoupon" class="input-xlarge required" style="width:264px">
			...
			<div class="span6">
				...
				<div class="controls">
					<form:select path="isCanUsePlusCoupon" class="input-xlarge required" style="width:264px">
						<form:option value="0" label="是"/>
						<form:option value="1" label="否"/>
					</form:select>
					<span class="help-inline"><font color="red">*</font></span>
		...
		<!-- 核心企业住所及电子签章. -->
		<div id="replace_repay_residence_id" style="display: none;">
			<div class="control-group">
				<div class="span10">
					<label class="control-label">核心企业住所：</label>
					<div class="controls">
						<form:textarea path="replaceRepayResidence" htmlEscape="false" class="input-xxlarge required" rows="2" />
						<span class="help-inline"><font color="red">&nbsp;&nbsp;&nbsp;*</font> </span>
					</div>
				</div>
			</div>
