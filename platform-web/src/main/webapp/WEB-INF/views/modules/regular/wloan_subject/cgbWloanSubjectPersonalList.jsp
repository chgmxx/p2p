		...
		<tbody>
			<c:forEach items="${page.list}" var="wloanSubject">
				<tr>
					...
					<td>${wloanSubject.loanBankNo}</td>
					<td>${wloanSubject.loanBankPhone}</td>
					<td>${wloanSubject.loanBankName}</td>
					<td>${wloanSubject.loanBankCode}</td>
					...
					<shiro:hasPermission name="wloan_subject:wloanSubject:edit">
						<td>
							<a href="${ctx}/wloan_subject/wloanSubject/cgbWloanSubjectForm?id=${wloanSubject.id}">修改</a>
							<a href="${ctx}/wloan_subject/wloanSubject/cgbDelete?id=${wloanSubject.id}" onclick="return confirmx('确认要删除该融资主体吗？', this.href)">删除</a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
