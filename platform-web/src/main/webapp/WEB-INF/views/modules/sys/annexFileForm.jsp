...
<head>
	...
	<script type="text/javascript">
		...
		function checkAnnexFile(){
			$.ajax({
				url : "${ctx}/sys/annexfile/checkForm?otherId="+$("#otherId").val()+"&value="+$('#type').val(), 
				type : "post", 
	...
	<form:form id="inputForm" modelAttribute="annexFile" action="${ctx}/sys/annexfile/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			<c:if test="${empty annexFile.id}">
				<input id="btnSubmit" class="btn btn-primary" type="button" onclick="checkAnnexFile()" value="保 存"/>&nbsp;
			</c:if>
